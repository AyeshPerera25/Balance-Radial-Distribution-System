...
def backwardTfYgD(Vp, Is, Z):
    ...
    # Apply phase shift (30 angle used)
    Is_sequential = Is_sequential * complex(math.cos(math.pi / 6), math.sin(math.pi / 6))
...
def fowardTfYgD(Vp, Vs, Is, Z):
    ...
    # Apply phase shift
    Vs_sequential = Vs_sequential * complex(math.cos(math.pi / 6), math.sin(math.pi / 6))
...
def forwardTfYYg(voltage, current, Z):
    Z_approx = numpy.array([[Z,0,0],[0,Z,0],[0,0,Z]])
    V = voltage - numpy.dot(Z_approx,current)
    return V